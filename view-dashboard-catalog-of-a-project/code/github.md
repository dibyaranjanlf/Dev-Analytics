...
The GitHub dashboards are available from the **Code** drop-down list.

### GitHub &gt; **Pull Requests**

SUMMARY: Pull Requests shows all the information about pull requests for a project.
...
**Backlog** shows a bar graph that represents the number of pull requests by open days over time \(confusing- Agnies to review\). Mouse over a color in the graph to see the total number of pull requests for the organization \(org name not showing- Agnies to review\) on a date.

**Backlog: Pull Requests \(Accumulated Time\)** shows a bar graph that represents the number of issues by accumulated open days over time. Mouse over a color in the graph to see the accumulated number of days pull requests were waiting to be closed on a date \(confusing- Agnies to review\) .
...
SUMMARY: PR Efficiency offers a view of efficiency closing issues based on metrics: Review Efficiency Index \(REI\), Time to Merge, and Lead Time.
...
SUMMARY: Timing shows information about open and closed pull requests in time. The dashboard shows submitters, repositories and organizations, but focuses on how long pull requests remain open. Statistical information provides closing times and also tables with the latest and the oldest pull requests.
